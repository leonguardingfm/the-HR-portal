...
import type { ScreeningPeriodYears } from "@/lib/types";
 ...
 */
export function VettingClockBoard({ files }: { files: ClockRow[] }) {
  const rows = files
    .map((file) => ({
      file,
      clock: clockState({
        conditionalEmploymentStart: file.conditionalEmploymentStart.toISOString(),
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[54rem] border-collapse text-left">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr>
                <td colSpan={8} className="py-6 text-center text-[12px]" style={{ color: "var(--text-muted)" }}>
                  No file is on the clock. It starts when a PIN is allocated and conditional employment begins.
                </td>
              </tr>
            )}
            {rows.map(({ file, clock }) => (
              ...
              >
                <td className="py-2.5 pr-3">
                  <Link href={`/vetting/${file.id}`} className="text-[13px] font-medium hover:underline">
                    {file.name}
                  </Link>
                  {file.extensionWeeks > 0 && (
                    <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>
                      +{file.extensionWeeks}wk extension approved
                    </p>
                  )}
                </td>
                <td className="tnum py-2.5 pr-3 text-[12px] tabular-nums" style={{ color: "var(--text-secondary)" }}>
                  {weeksAllowed(file.screeningPeriodYears as ScreeningPeriodYears)} wk
                ...
                <td className="w-40 py-2.5 pr-3">
                  <Meter
                    ...
                    label={`${file.name}: ${Math.round(clock!.fractionUsed * 100)}% of the allowed period used`}
                  />
                </td>
                <td className="py-2.5 pr-3">
                  <StatusPill
