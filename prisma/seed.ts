...
import { DEPARTMENTS, DEV_SEED_PASSWORD } from "../lib/accounts";
...
/**
 * Username, email and department for a demonstration account. The department
 * is the one whose role is the account's first role, or none where that role
 * is not one people register for (higher management, finance).
 */
function seedLogin(u: { name: string; personId: string; roles: string[] }) {
  const slug = u.name.toLowerCase().replace(/[^a-z0-9]+/g, ".").replace(/^\.|\.$/g, "");
  return {
    username: slug,
    email: `${slug}@example.com`,
    department: DEPARTMENTS.find((d) => d.role === u.roles[0])?.id ?? null,
  };
}
...
async function seedPeople() {
  ...
  // Username and password sign-in. Every demonstration account shares one
  // development password, shown on the sign-in page outside production.
...
async function seedVetting() {
  ...
  // Accepted, so the file is back in full screening rather than paused.
  ...
  await db.screeningFile.update({
    where: { id: screeningFiles[2].id },
    data: { status: "full_screening_in_progress" },
  });
}
