...
```
 ...
 TRACK B — RECRUITMENT  (Recruitment)
 Sourcing ─> Shortlisted ─> Invited ─> Application received ─> Application complete
   ...
   ─> Onboarding complete = DEPLOYABLE ─> ((GATE 2)) ─> Confirmed employment

 TRACK C — VETTING  (Vetting admin + Vetting controller)
 Not started ─> Consent captured ─> Information complete ─> Preliminary checks done
   ─> Limited screening done ─> CONTROLLER REVIEW #1 ══> unlocks GATE 1
   ─> Full screening in progress  [12 or 16 WEEK CLOCK RUNNING]
   ─> Full screening complete ─> CONTROLLER REVIEW #2 ══> unlocks GATE 2
...
These are the only places the two tracks touch, and they are the reason the portal exists.

### GATE 1 — no conditional offer without limited screening
A conditional offer (and therefore the Welcome Pack, the Employment Contract, and any deployment
to a client site) is **blocked** until all four are true:

1. Risk in the intended role evaluated, deemed acceptable, and **documented** [7.5.1a]
2. Preliminary checks complete [7.4]
...
The portal should show the block as a plain sentence — "Conditional offer blocked: awaiting
controller review (limited screening)" — not a greyed-out button with no explanation.

On passing Gate 1 the portal **starts the clock**: it records the conditional employment
commencement date, computes the 12 or 16 week deadline from the screening period, and displays
both, along with the date employment must cease if screening does not complete [7.2, 7.6].

### GATE 2 — no confirmed employment without full screening
Confirmed employment is blocked until the whole screening period is verified with no unverified
gap over 31 days [7.7], the criminality requirement is satisfied [7.7j], and the controller has
reviewed the completed file. The standard is unambiguous: no offer of confirmed employment unless
full screening has completed satisfactorily [7.7].

If the clock expires without completion, the file moves to **Time expired** and the officer is
flagged as not to continue in relevant employment [7.6]. That flag needs to reach Control, because
...
|-------|-------|-----------|--------|
| A1 Received | Control | Client, site, post, headcount, shift pattern, start date, **screening period required (5 or 10 yr)** | Logged same working day |
...
## 4. Track B — Recruitment (owner: Recruitment)
...
| B3 Invited | Recruitment | Application link sent with instructions; **consent and declaration now part of the form** (see 01 §3.2) | Same day as B2 |
...
| B9 Signed docs complete | Recruitment | All required signatures received and checked | Chase at **48h**, **5 days**, escalate at **10 days** |
| B10 Onboarding complete | Recruitment | The eight-item checklist in §6 below | **2 working days** of B9 |
| — | — | **GATE 2** | — |
| B11 Confirmed employment | Vetting controller → Recruitment | Full screening complete and reviewed; status updated; officer record finalised | On Gate 2 clearing |

## 5. Track C — Vetting (owners: Vetting Administrator, then Vetting Controller)

| Stage | Owner | Checklist | Target |
|-------|-------|-----------|--------|
| C1 Consent captured | Vetting admin | Authorisation to approach employers, government departments, educational establishments and a credit reference agency [7.3.2f]; signed declaration [7.3.2g]; **permission-to-contact flag recorded per employer**, current employer excluded unless permitted [7.7b] | Automatic on application submission once consent moves into the form |
...
| C6 Full screening | Vetting admin | Whole screening period verified; **no unverified period over 31 days** [7.7]; education leaving date [7.7a]; employment periods and type [7.7b]; registered unemployment via DWP [7.7c]; self-employment [7.7d]; career breaks [7.7e]; residence abroad [7.7f]; travel abroad over 31 days [7.7g]; incomplete-record evidence [7.7h]; criminality element [7.7j] | **HARD: 12 weeks** (5-yr) / **16 weeks** (10-yr) from conditional employment start [7.6] |
| C7 Controller review #2 | Vetting **controller** | Completed file reviewed; if screening was outsourced, **our controller still reviews the provider's file** [6.3] | Within the clock |

**Reference chasing rhythm inside C6** (proposed):
