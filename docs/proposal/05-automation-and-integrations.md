...
| **INDEL** | Hand the officer over at onboarding: create the profile from what the portal already holds, rather than retyping it | **An integration, not a replacement.** The portal takes the HR part; shift assignment and operational deployment stay in INDEL, and the boundary is the onboarding handover. Whether it has an API is still open — C17. Fallback is a structured export |
